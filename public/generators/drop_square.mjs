...
//let dropParams = {dropTries:0,maxDrops:0}
...
rs.generateDrop = function (p,rvs) {
 ...
 // console.log('x',x,'y',y,'sy',sy,'vcross',vcross);
  ...
  let omd = (Math.abs(y) < 0.25*wd)&&(Math.abs(x) < 0.25*wd);
  let msv = Math.abs(x) < 0.125*wd
  let msh = Math.abs(y) < 0.125*wd
  let v = rvs.v;
  let clr = `rgb(${v},${v},${v})`;
  let p0 = Point.mk(0,0);
  let {minSeparation,lineP} = this;
  let nd = md?1:(Math.abs(y)>0.125*wd?-1:1);
  if (!msv&&!msh) {
    return;
  }
  let sp = this.segParams(nd);
  let {length,angle} = sp;
  //if (cross &&  !md) {
