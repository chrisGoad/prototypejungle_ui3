...
rs.initialize = function () {
 ...
 // let P = Point.mk(20,0);
  //let r1 = 1;
  //let r2=3;
  let prt1 = {ray,initialPosition:Point.mk(0,0);startTime:0,shape:circ1,mass:0.5,radius:1};
  let prt2 = {ray:{initialPosition:Point.mk(20,0),velocity:Point.mk(0,0)},startTime:0,shape:circ0,mass:1,radius:3};
  let prts = this.particles = [prt1,prt2];
  this.mkCirclesForParticles(prts);
  //let params = {A,V,P,r1,r2};
  let {ray:ray1,radius:radius1} = prt1;
  let {ray:ray2,radius:radius2} = prt2;

  let {initialPosition:ip1,velocity:v1} = ray1;
  let {initialPosition:ip2,velocity:v2} = ray2;
  let params = {ray1,v1,ip2,radius1,radius2};
