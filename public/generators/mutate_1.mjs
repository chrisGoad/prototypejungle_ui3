...
rs.adjustParamsAforGridCell = function (i,j,paramsA) {
 ...
 // let idx = this.gridCellIndex(i,j)*12;
  let idx = this.gridCellIndex(i,j)*2*pln;
  paramsA.forEach((pa) => {
    let h =pa.horizontal;
    pa.center = c;
    pa.index = idx+(h?0:pln);
  });
  return paramsA;
}
...
rs.paramsAselect = function (paramsA,k) {
  return [paramsA[2*k],paramsA[2*k+1]]
  if (k===0) {
...
rs.setCellState = function (i,j) {
 ...
 for (let n=0;n<pln;n++) {
   if (inInterval(ssf,n)) {
     let fr = fractionThruInterval(ssf,fr0); 
     if (fr <0.5) {
      this.clines(paramsA,n*2,n*2+1,2*fr);
    } else {
      this.clines(paramsA,n*2+1,(n+1)*2,2*(fr-0.5));
    }  
  }    
 ...
 /* if (inInterval(ssf,iv0) || inInterval(ssf,iv2)) {
  ...
  if (inInterval(cyssf,iv1)) {
    ...
    if (fr <0.5) {
      this.clines(paramsA,0,1,2*fr);
    } else {
      this.clines(paramsA,1,2,2*(fr-0.5));
    }    
  }
  if (inInterval(cyssf,iv3)) {
    let fr = fractionThruInterval(cyssf,iv3);
    if (fr <0.5) {
      this.clines(paramsA,2,1,2*fr);
    } else {
      this.clines(paramsA,1,0,2*(fr-0.5));
    }    
...
rs.initialize = function () {
 ...
 // paramsh.push({index:6,center:Point.mk(0,0),horizontal:1,lineLength:8,lineSep:4,lineDist:6});
  ...
  this.clines(0,1,0);*/
  //this.clines(paramsA,0,1,0);
  this.setCellStates();
 //this.clinesForGrid(0,1,0);
...
//  debugger;
  ...
  let hns = ups/2;
  let  stepLn = ns/(pln-1);
  const ivi = (i) => [stepLn*i,stepLn*(i+1)];
  /*let iv0 = [0,hps];//pause
  let iv1 = [hps,hps+hups]; 
  let iv2 = [hps+hups,ps+hups];//pause
  let iv3 = [ps+hups,ps+ups];*/
  ...
  if (inInterval(ssf,n)) {
    ...
    console.log('n',n'fr',fr);
    if (fr <0.5) {
      //this.clines(0,1,2*fr);
      this.clines(paramsA,0,1,2*fr);
      this.clinesForGrid(0,1,2*fr);
    } else {
     // this.clines(1,2,2*(fr-0.5));
      //this.clines(paramsA,1,2,2*(fr-0.5));
      this.clinesForGrid(1,2,2*(fr-0.5));
    }    
  }
  if (inInterval(ssf,iv3)) {
    ...
    if (fr <0.5) {
     ...
     // this.clines(paramsA,2,1,2*fr);
      this.clinesForGrid(2,1,2*fr);
    } else {
     // this.clines(1,0,2*(fr-0.5));
      //this.clines(paramsA,1,0,2*(fr-0.5));
      this.clinesForGrid(1,0,2*(fr-0.5));
    }    
