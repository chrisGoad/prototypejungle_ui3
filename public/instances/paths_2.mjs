...
let topParams = {width:wd,height:wd,numRows:nr,numCols:nr,framePadding:.1*wd,stepsPerMove:10,numStepss:24,numSteps:300,center:Point.mk(0,0),radius:wd/4,
                 cycles:1,frameStroke:'rgb(2,2,2)',frameStrokee:'white',frameStrokeWidth:1,saveAnimation:1,stepInterval:40,scaling:wd,
                 pauseAtt:[29,30,59,60],numConnections:160,numPhases:200,showThePaths:0,showIntersections:0,newCoords:1}
...
rs.addMotions = function () {
  ...
  let d4 = d3+.05;
  let df = Point.mk(.5,.5);
  let crn0 = Point.mk(0,0).difference(df);
  let crn1 = Point.mk(d1,d1).difference(df);
  let crn2 = Point.mk(d2,d2).difference(df);
  let crn3 = Point.mk(d3,d3).difference(df);
  let crn4 = Point.mk(d4,d4).difference(df);
  ...
  let path3 = this.mkRectangularPath(rect3).reverse();
    ...
    this.thePath = path0;
  let paths = [path0,path1,path2,path3,path4];
  this.thePaths = paths;
