...
let topParams = {width:wd,height:wd,numRows:nr,numCols:nr,framePadding:.1*wd,stepsPerMove:10,numStepss:24,numSteps:1000,center:Point.mk(0,0),radius:wd/4,
                ...
                // pauseAtt:[29,30,59,60],numConnections:25,numPhases:50,showThePath:0}
                 pauseAtt:[29,30,59,60],numConnections:50,numPhases:150,showThePath:0,showIntersections:1}
...
rs.connectIndices = function (params) {
  ...
  let e0si = ci;//Math.floor(Math.random()*ln);
  let e1pi = pi;
  let e1si = (e0si + 20)%ln;
...
rs.addMotions = function () {
 // debugger;
  ...
  let path = this.thePath = this.mkSnakePath({numPoints:500,numTurns:7});
